...
public class TransferCompletedConsumer : IConsumer<TransferCompletedEvent>
{
    ...
    public TransferCompletedConsumer(ILogger<TransferCompletedConsumer> logger) => _logger = logger;
    ...
    public Task Consume(ConsumeContext<TransferCompletedEvent> context)
    {
        ...
        _logger.LogInformation($"[RABBITMQ] Notificação: A transação {message.TransactionId} de R$ {message.Amount} foi processada com sucesso.");
